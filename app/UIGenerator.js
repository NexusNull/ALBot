...
class UIGenerator {
    ...
    generateMiniMap(hitLog, entities) {
        ...
        if (this.gameContext.character) {


            var screenSize = {width: 800, height: 600};
            var pos = {
                x: this.gameContext.character.real_x - Math.floor(screenSize.width / 2),
                y: this.gameContext.character.real_y - Math.floor(screenSize.height / 2)
            };
            var map = this.gameContext.character.in;

            if (this.gameContext.G.maps[map]) {
                let xLines = this.gameContext.G.maps[map].data.x_lines;
                let yLines = this.gameContext.G.maps[map].data.y_lines;
            ...
            for (let hit of hitLog) {
                ...
                if (hit.hid === this.gameContext.character.id)
                    source = this.gameContext.character;
                if (hit.id === this.gameContext.character.id)
                    target = this.gameContext.character;
            ...
            var targetIds = [];
            if (this.gameContext.character.target)
                targetIds.push(this.gameContext.character.target);
            for (let id in entities) {
                let entity = entities[id];
                if (entity.target === this.gameContext.character.id)
            ...
            pngUtil.draw_dot(
                ((this.gameContext.character.real_x - pos.x) / screenSize.width) * png.width,
                ((this.gameContext.character.real_y - pos.y) / screenSize.height) * png.height,
                2,
                png, [0, 200, 0, 255]);
