...
function draw_funny_boxes({boxMap, size, map, qtn, qtg, filename}) {
    var fs = require('fs'),
    ...
    try {
        ...
        for (let j = 0; j < boxMap.length; j++) {
            if (boxMap[j] !== 0) {
                let color = colors[(boxMap[j] - 1) % (colors.length - 1)];
                let idx = j << 2;
        ...
        /*
        for (let j = 0; j < world.qtn.length; j++) {
            let node = world.qtn[j];
            if (world.qtg[node.x + ":" + node.y]) {
                for (let edge of world.qtg[node.x + ":" + node.y]) {
...
module.exports = {draw_funny_boxes, draw_funny_lines,draw_box_path};
