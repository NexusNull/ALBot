...
function draw_funny_boxes(floor, list) {
    ...
    try {
        for (let j = 0; j < floor.matrices.base.length; j++) {
            if (floor.matrices.base[j] == 0xffff) {
                const idx = j << 2;
                png.data[idx] = 0;
                png.data[idx + 1] = 0;
                png.data[idx + 2] = 0;
                png.data[idx + 3] = 255;
            } else if (floor.matrices.base[j] == 0xfffe) {
                const idx = j << 2;
                png.data[idx] = 255;
                png.data[idx + 1] = 255;
                png.data[idx + 2] = 255;
                png.data[idx + 3] = 255;
            } else if (list.has(floor.matrices.base[j])) {
                const color = colors[((floor.matrices.base[j]) % 31)];
                const idx = j << 2;
