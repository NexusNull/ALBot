...
function draw_funny_boxes(floor) {
    var png = new PNG({
        width: floor.size.x,
        height: floor.size.y,
        filterType: -1
    ...
    try {
        for (let j = 0; j < floor.matrices.base.length; j++) {
            if (floor.matrices.base[j] == 2) {
                let idx = j << 2;
                png.data[idx] = 255;
                png.data[idx + 1] = 255;
                png.data[idx + 2] = 255;
        ...
        for (let j = 0; j < floor.matrices.boxMap.length; j++) {
            if (floor.matrices.boxMap[j] !== 0) {
                let color = colors[(floor.matrices.boxMap[j] - 1) % (colors.length - 1)];
                let idx = j << 2;
                png.data[idx] = Math.floor(Math.min(png.data[idx] * (1 / 2) + color[0] * (1 / 2), 255));
                png.data[idx + 1] = Math.floor(Math.min(png.data[idx] * (1 / 2) + color[1] * (1 / 2), 255));
                png.data[idx + 2] = Math.floor(Math.min(png.data[idx] * (1 / 2) + color[2] * (1 / 2), 255));
                png.data[idx + 3] = 255;
            }
        }

        /*
        for (let j = 0; j < floor.qtn.length; j++) {
            let node = floor.qtn[j];
            if (floor.qtg[node.x + ":" + node.y]) {
                for (let edge of floor.qtg[node.x + ":" + node.y]) {
...
*/
    ...
    fs.writeFileSync('./pathfinding/maps/' + floor.name + ".png", buffer);
}
