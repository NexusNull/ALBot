...
function add_chat(b, k, d) {
    ...
    if (game_chats.length > 240) {
        var j = "<div class='chatentry' style='color: gray'>- Truncated -</div>", a = [];
...
function skill_timeout(b, a) {
    ...
    draw_trigger(function () {
        if (c) {
            ...
            add_tint(".skidloader" + c, {ms: -mssince(next_skill[b]), type: "skill"})
        }
    })
}
...
function add_name_tag(c) {
    ...
    var a = "Lv." + c.level + " " + c.name, h = 7433580, b = a.length * 4 + 4, j = 11, k = 1;
    if (c.npc) {
        a = c.name || "NPC";
        h = 2531789;
        if (c.citizen) {
    ...
    var d = {fontFamily: S.font, fontSize: 64 * k, fill: "white", align: "center"};
...
function d_text(n, j, h, g) {
                                ...
                                if (b == "sneak") {
                                    ...
                                    if (b == "evade") {
                                        ...
                                        if (l && g.from && get_entity(g.from)) {
                                            d_line(get_entity(g.from), l, {color: "evade"})
                                        }
                                    } else {
                                        if (b == "reflect") {
                                            b = "#6D62A2"
    ...
    function d(p, q) {
        return function () {
            var r = mssince(q.last_fade), t = round(4 * r / 100);
            if (2 < t && t < 7) {
                t = 4
            }
            q.y -= t;
            ...
            if (q.alpha > 0.25) {
                ...
                remove_sprite(q);
                try {
                    q.destroy({texture: true, baseTexture: true})
                } catch (s) {
                }
            }
        }
